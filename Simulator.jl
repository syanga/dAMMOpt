...
function ad_hoc_policy(adjust_C, adjust_k)
    function _ad_hoc_policy(res::Scenario, forecast::Matrix, est_net_position::Vector)
        C, k, pₘ, pₒ, x, net_pos = res.C[end], res.k[end], res.mark[end], res.oracle[end], res.x[end], res.net_position[end]
        LIQ_LIMIT = 0.05
        ...
        if adjust_C && repeg_cost > -20
            ...
            pred_funding_cost = sum(funding_cost(p̂ₘ, p̂ₒ, est_net_pos) for (p̂ₘ, p̂ₒ, est_net_pos) in zip(forecast[:, 3], forecast[:, 4], est_net_position))
        ...
        if t > H
            k_bias = sum((res.mark[end-i] .- res.oracle[end-i])  * (res.net_position[end-i] > 0)  for i = 1:H)
